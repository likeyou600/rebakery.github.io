...
這種按鍵適合拿來做 A / B / Start / Back 這類功能鍵。
...
一開始可以只用 polling + timestamp 做 debounce。
這個版本最直覺，適合先確認 GPIO 接線、按鍵狀態和 input event 流程。

但按鍵輸入也很適合練 FreeRTOS 的另一個常見場景：

> interrupt 發生後，不在 ISR 裡做太多事情，而是通知 task 處理。

所以這篇會分成兩個版本：

第一版是 **polling debounce**。
`input_task` 固定週期掃描 GPIO，用 timestamp 判斷狀態是否穩定。

第二版是 **EXTI + software timer debounce**。
按鍵邊緣觸發 interrupt 後，ISR 只負責啟動 debounce timer。等 timer 到期，再通知 `input_task` 讀取穩定後的按鍵狀態。
...
{% codeblock lang:c line_number:true %}
static void input_scan_button(input_button_hw_t *hw,
...
while (1)
{
osThreadFlagsWait(INPUT_FLAG_BUTTON_IRQ,
osFlagsWaitAny,
osWaitForever);
...
| 做法                  | 優點                   | 缺點                                         | 適合階段        |
| --------------------- | ---------------------- | -------------------------------------------- | --------------- |
| Polling + timestamp   | 簡單、直覺、好 debug   | input_task 需要週期性醒來掃描                | 第一版 bring-up |
| EXTI + software timer | 事件驅動、不必一直掃描 | 流程較複雜，需要處理 ISR、timer、task notify | 進階版本        |

第一版我會先用 polling，把所有按鍵流程跑通。
等 input system 穩定後，再考慮把部分按鍵改成 EXTI-driven。
